...
private def addOp : Utf8Parser Error conditional (Expr → Expr → Expr) :=
  Expr.add <$ᵍ lexeme (char '+')
  <|> Expr.sub <$ᵍ lexeme (char '-')

private def mulOp : Utf8Parser Error conditional (Expr → Expr → Expr) :=
  Expr.mul <$ᵍ lexeme (char '*')
  <|> Expr.div <$ᵍ lexeme (char '/')

def expr : Utf8Parser Error conditional Expr :=
  fix (fun expr_rec =>
