...
-- chainl1
private def plus : Utf8Parser Error conditional (Nat → Nat → Nat) := gdo
  let _ ← satisfy (· == '+')
  return (· + ·)
...
-- withRecovery
private def recoverDigit : Error → Utf8Parser Error conditional Nat :=
  fun _ => digit
...
-- tryResume
private def alwaysFail : Utf8Parser Error conditional Char := satisfy (fun _ => false)
